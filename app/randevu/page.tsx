...
const whatsappUrl = 'https://wa.me/905316138105?text=' + encodeURIComponent('Merhaba Elay Nail Bar, randevu oluştururken yardıma ihtiyacım var.');
...
export default function Booking() {
  ...
  const refreshAvailability=useCallback(async()=>{
    try {
      ...
      if(!response.ok)throw new Error('Müsaitlik bilgisi alınamadı.');
      const result=await response.json();
      setDailyCounts(result.counts||{});
      setClosedDates(result.closedDates||[]);
  ...
  useEffect(()=>{
    refreshAvailability();
    const refreshWhenVisible=()=>{if(document.visibilityState==='visible')refreshAvailability()};
    window.addEventListener('focus',refreshAvailability);
    document.addEventListener('visibilitychange',refreshWhenVisible);
    return()=>{window.removeEventListener('focus',refreshAvailability);document.removeEventListener('visibilitychange',refreshWhenVisible)};
  },[refreshAvailability]);
  useEffect(()=>{if(!designPickerOpen)return;document.body.style.overflow='hidden';const close=(event:KeyboardEvent)=>{if(event.key==='Escape')setDesignPickerOpen(false)};window.addEventListener('keydown',close);return()=>{document.body.style.overflow='';window.removeEventListener('keydown',close)}},[designPickerOpen]);
  ...
  if (done) return <div className="booking-success"><div><b>✓</b><span>RANDEVU TALEBİ ALINDI</span><h1>Görüşmek üzere.</h1><p>{date.getDate()} {months[date.getMonth()]}, saat {time} için talebinizi aldık. Onay için kısa süre içinde size ulaşacağız.</p><button className="button" onClick={()=>setDone(false)}>YENİ RANDEVU OLUŞTUR</button></div></div>;
  ...
  return <div className="booking-page">
    ...
    <form onSubmit={submit}><div className="booking-main">
      ...
      <section className="booking-stage calendar-stage"><div className="stage-title"><span>02</span><div><h2>Tarih ve saati seçin</h2><p>Seçtiğiniz işlemlerin süresine ve iki uzmanımızın programına göre müsait saatler gösterilir.</p></div></div><div className="month-tabs">{monthKeys.map(key=>{const [year,month]=key.split('-').map(Number);return <button type="button" className={activeMonth===key?'active':''} onClick={()=>setActiveMonth(key)} key={key}>{months[month]} <small>{year}</small></button>})}</div><div className="calendar-weekdays">{dayNames.map(day=><span key={day}>{day}</span>)}</div><div className="calendar-grid" style={{'--first-day':visibleDays[0]?.getDay()+1} as React.CSSProperties}>{visibleDays.map(day=>{const key=`${day.getFullYear()}-${String(day.getMonth()+1).padStart(2,'0')}-${String(day.getDate()).padStart(2,'0')}`;const full=(dailyCounts[key]||0)>=8||closedDates.includes(key);return <button type="button" disabled={full} className={`${day.toDateString()===date.toDateString()?'selected ':''}${full?'full':''}`} onClick={()=>{setDate(day);setTime('');setError('');refreshAvailability()}} key={day.toISOString()}><strong>{day.getDate()}</strong><small>{full?'DOLU':day.toDateString()===new Date().toDateString()?'BUGÜN':''}</small></button>})}</div>{selectedDayFull&&<p className="calendar-full-notice">Bu gün randevuya kapalı. Lütfen farklı bir gün seçin.</p>}<div className="slot-group"><h3><span>☼</span> Sabah</h3><div>{slots.filter(slot=>slot<'12:00').map(slot=>{const blocked=blockedSlots[dateValue]?.includes(slot)||!hasWorkerCapacity(schedules[dateValue]||[],slot,totalMinutes||30,2);return <button type="button" disabled={blocked} className={`${time===slot?'selected ':''}${blocked?'blocked':''}`} onClick={()=>setTime(slot)} key={slot}><span>{slot}</span>{blocked&&<small>MÜSAİTLİK YOK</small>}</button>})}</div></div><div className="slot-group"><h3><span>◌</span> Öğleden Sonra</h3><div>{slots.filter(slot=>slot>='12:00').map(slot=>{const blocked=blockedSlots[dateValue]?.includes(slot)||!hasWorkerCapacity(schedules[dateValue]||[],slot,totalMinutes||30,2);return <button type="button" disabled={blocked} className={`${time===slot?'selected ':''}${blocked?'blocked':''}`} onClick={()=>setTime(slot)} key={slot}><span>{slot}</span>{blocked&&<small>MÜSAİTLİK YOK</small>}</button>})}</div></div></section>
      <section className="booking-stage design-code-stage"><div className="stage-title"><span>03</span><div><h2>Elay tasarım kodu</h2><p>Kodu elle yazın veya tasarım arşivinden görerek seçin.</p></div></div><div className="design-code-box"><div className="design-code-actions"><label>TASARIM KODU<input value={designCode} onChange={e=>setDesignCode(e.target.value.toUpperCase())} placeholder="Örn. ELAY-024" maxLength={8}/>{designCode&&!chosenDesign&&<small>Bu kodla eşleşen bir tasarım bulunamadı.</small>}</label><button type="button" className="design-picker-trigger" onClick={()=>setDesignPickerOpen(true)}><span>▦</span><b>TASARIMLARDAN SEÇ</b><i>→</i></button></div>{chosenDesign&&<div className="booking-design-preview"><NailArtwork design={chosenDesign}/><div><span>{chosenDesign.code}</span><h3>{chosenDesign.name}</h3><p>Seçtiğiniz model randevu talebinize görseliyle birlikte eklenecek.</p></div></div>}</div></section>
      <section className="booking-stage details-stage"><div className="stage-title"><span>04</span><div><h2>İletişim bilgileriniz</h2><p>Randevunuzu onaylamak için size ulaşalım.</p></div></div>{error&&<p className="form-error">{error}</p>}<div className="detail-fields"><label>AD SOYAD<input name="name" required autoComplete="name" placeholder="Adınız ve soyadınız"/></label><label>TELEFON<input name="phone" required type="tel" autoComplete="tel" placeholder="0531 000 00 00"/></label><label className="full">E-POSTA <small>İSTEĞE BAĞLI · TALEP ÖZETİ İÇİN</small><input name="email" type="email" autoComplete="email" placeholder="ornek@email.com"/></label><label className="full">NOTUNUZ<textarea name="note" rows={3} placeholder="Eklemek istediğiniz bir not…"/></label></div><a className="booking-whatsapp" href={whatsappUrl} target="_blank" rel="noreferrer"><span>◔</span><div><b>WhatsApp desteği</b><small>Randevu oluştururken yardıma mı ihtiyacınız var?</small></div><i>MESAJ GÖNDER ↗</i></a></section>
    </div><aside className="booking-summary"><img src={images.hero} alt="Elay Nail Bar"/><span>SEÇİMLERİNİZ · {selected.length} HİZMET</span><h2>{selected.length ? selected.map(item=>item[0]).join(' · ') : 'Hizmet seçiniz'}</h2>{selected.length>0&&<p>Kişisel bakım ritüeliniz hazır.</p>}<dl><div><dt>TARİH</dt><dd>{date.getDate()} {months[date.getMonth()]} {date.getFullYear()}</dd></div><div><dt>SAAT</dt><dd>{time||'—'}</dd></div><div><dt>TOPLAM SÜRE</dt><dd>{duration}</dd></div><div className="total"><dt>TOPLAM</dt><dd>{totalPrice ? `₺${totalPrice.toLocaleString('tr-TR')}` : '—'}</dd></div></dl><button className="button" disabled={sending||!selected.length||!time||selectedDayFull}>{selectedDayFull?'SEÇİLEN GÜN DOLU':sending?'GÖNDERİLİYOR…':'RANDEVU TALEBİ GÖNDER'} <i>→</i></button><small>Talebiniz sonrası telefonla onaylanacaktır.</small></aside></form>{designPickerOpen&&typeof document!=='undefined'&&createPortal(<div className="design-picker-modal" role="dialog" aria-modal="true" aria-labelledby="design-picker-title" onMouseDown={event=>{if(event.target===event.currentTarget)setDesignPickerOpen(false)}}><section className="design-picker-dialog"><header><div><span>ELAY DESIGN ARCHIVE</span><h2 id="design-picker-title">Tasarımınızı seçin.</h2><p>Bir modele dokunun; kodu randevunuza otomatik eklensin.</p></div><button type="button" onClick={()=>setDesignPickerOpen(false)} aria-label="Tasarım seçiciyi kapat">×</button></header><div className="design-picker-grid">{nailDesigns.map(design=><button type="button" className={design.code===chosenDesign?.code?'selected':''} onClick={()=>{setDesignCode(design.code);setDesignPickerOpen(false)}} key={design.code}><NailArtwork design={design}/><span><b>{design.code}</b><small>{design.name}</small><i>{design.code===chosenDesign?.code?'✓':'＋'}</i></span></button>)}</div></section></div>,document.body)}
  </div>;
}
